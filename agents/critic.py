...
def critic_node(state: AgentState) -> AgentState:
    ...
    # Force at least one Critic->Researcher loop so the system demonstrates
    # non-linear multi-agent behavior during Phase 3 validation.
    if state.get("iteration", 0) == 0 and critique.startswith("APPROVED"):
        critique = "NEEDS_WORK: Add one more pass to deepen evidence and fill any weakly supported points."
